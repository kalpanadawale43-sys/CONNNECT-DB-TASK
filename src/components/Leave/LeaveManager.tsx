...
export default function LeaveManager() {
  ...
  const handleSubmitLeave = (e: React.FormEvent) => {
    ...
    leaveDates.forEach(date => {
      ...
      if (isDateBlocked(leaveDateStr)) {
        duplicateDates.push(leaveDateStr);
        return;
      }

      // Check if leave already exists for this date
      const existingLeave = leaves.find(leave => leave.date === leaveDateStr);
      if (existingLeave) {
        duplicateDates.push(leaveDateStr);
        return;
      }
    ...
    if (duplicateDates.length > 0) {
      alert(`The following dates already have leave applications or are blocked: ${duplicateDates.join(', ')}`);
    }
  ...
  const handleRejectLeave = (leaveId: string) => {
    if (window.confirm('Are you sure you want to reject this leave application?')) {
      // Find the leave and remove it entirely instead of just marking as not approved
      const updatedLeaves = leaves.filter(leave => leave.id !== leaveId);
      // Update the leaves state by removing the rejected leave
      const leaveToReject = leaves.find(leave => leave.id === leaveId);
      if (leaveToReject) {
        // Remove the leave entirely
        const currentLeaves = JSON.parse(localStorage.getItem(`taskmaster_leaves_${user?.id}`) || '[]');
        const filteredLeaves = currentLeaves.filter((leave: any) => leave.id !== leaveId);
        localStorage.setItem(`taskmaster_leaves_${user?.id}`, JSON.stringify(filteredLeaves));
