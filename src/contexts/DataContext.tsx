...
export function DataProvider({ children }: { children: React.ReactNode }) {
  ...
  const loadUserData = () => {
    ...
    // Load data into local variables to process before setting state
    const loadedSessions = savedSessions ? JSON.parse(savedSessions) : [];
    const loadedTasks = savedTasks ? JSON.parse(savedTasks) : [];
    let loadedLeaves = savedLeaves ? JSON.parse(savedLeaves) : [];
    ...
    // Synchronously check and add mandatory leaves before setting state to avoid race conditions
    if (user.startDate) {
      ...
      for (let i = loadedSettings.dayOffInterval; i <= totalDays; i += loadedSettings.dayOffInterval) {
        const mandatoryDate = addDays(startDate, i);
        const mandatoryDateStr = format(mandatoryDate, 'yyyy-MM-dd');

        const leaveExists = loadedLeaves.some(leave => leave.date === mandatoryDateStr && leave.type === 'mandatory');
      ...
      if (newLeaves.length > 0) {
        loadedLeaves = [...loadedLeaves, ...newLeaves];
        saveData('leaves', loadedLeaves); // Persist the updated leaves list
      }
    }

    // Set all states once after processing is complete
